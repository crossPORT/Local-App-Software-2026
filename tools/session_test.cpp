...
int run_receiver(int recv_port,
    ...
    SessionOrchestrator receiver(
        rocketbox::make_shared_transport(rocketbox::TransportMode::Usb, recv_port),
        identity,
...
int main(int argc, char* argv[]) {
    ...
    SessionOrchestrator sender(
        rocketbox::make_shared_transport(rocketbox::TransportMode::Usb, send_port),
        send_identity,
        [&](const OrchestratorUiState& state) {
    ...
    if (!sender.send_to_peer(target, {"/tmp/small-file.bin"}, "session test")) {
