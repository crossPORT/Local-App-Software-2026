// Command-line session peer — mirrors wx GUI session flow with file logging.
//
//   rocketbox-cli listen --port 1
//   rocketbox-cli send --port 0 --to Peer-Name --file /path
//   rocketbox-cli probe  --port 0
...
void usage(const char* argv0) {
    std::cerr
        ...
        << "\nLog file: " << event_log_path() << " (override: ROCKETBOX_LOG=/path)\n";
}
...
int run_listen(const Args& args) {
    ...
    SessionOrchestrator orchestrator(
        rocketbox::make_shared_transport(rocketbox::TransportMode::Usb, args.port),
        identity,
        [&](const OrchestratorUiState& state) {
            const std::string line = state_line(state);
            if (line != last_status) {
                last_status = line;
                event_log(args.port, "state", line);
            }
    ...
    event_log(args.port, "listen_stop", "");
    orchestrator.stop();
    return 0;
}
...
int run_send(const Args& args) {
    ...
    if (!orchestrator.send_to_peer(args.peer, {args.file}, args.note)) {
        event_log(args.port, "send_rejected", "orchestrator busy or peer missing");
        orchestrator.stop();
        return 1;
    }
    ...
    event_log(args.port, success ? "send_ok" : "send_fail", last_status);
    orchestrator.stop();
    return success ? 0 : 3;
}
...
int run_probe(const Args& args) {
    event_log_clear();
    event_log(args.port, "probe_start", "");
    ...
    SessionOrchestrator orchestrator(
        rocketbox::make_shared_transport(rocketbox::TransportMode::Usb, args.port), identity, [](const OrchestratorUiState& state) {
        if (!state.dev_log.empty()) {
            event_log(-1, "probe_dev", state.dev_log);
        }
    });
    ...
    event_log(args.port, "probe_done", "");
    return 0;
}
