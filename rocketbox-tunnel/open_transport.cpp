...
int resolve_tunnel_display_port(rocketbox::TransportMode mode, int display_port) {
  ...
  return resolve_usb_display_port(display_port);
}

std::unique_ptr<rocketbox::RocketBoxTransport> open_tunnel_transport(
    rocketbox::TransportMode mode, int display_port) {
  if (mode == rocketbox::TransportMode::Sim) {
    const int port = resolve_tunnel_display_port(mode, display_port);
    auto t = rocketbox::create_rocketbox_transport(mode, port);
    t->connect();
    return t;
  }

  const int want = resolve_tunnel_display_port(mode, display_port);
