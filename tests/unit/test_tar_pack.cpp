...
RB_TEST(tar_pack_multiple_files) {
    if (!command_available("tar")) {
        std::cerr << "    skipped: tar not available\n";
        return;
    }

    const std::filesystem::path staging = unique_test_dir("rocketbox-tar-test");
    std::error_code ec;
...
RB_TEST(tar_pack_disambiguates_same_basename) {
    if (!command_available("tar")) {
        std::cerr << "    skipped: tar not available\n";
        return;
    }

    const std::filesystem::path staging = unique_test_dir("rocketbox-tar-dup");
    std::error_code ec;
    std::filesystem::create_directories(staging, ec);
    CHECK(!ec);
