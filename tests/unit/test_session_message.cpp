...
std::string write_raw(const std::string& contents) {
    char tmpl[] = "/tmp/rocketbox-test-sess-XXXXXX";
...
RB_TEST(session_kind_string_round_trip) {
...
RB_TEST(session_offer_write_read_round_trip) {
    SessionMessage msg;
...
RB_TEST(session_write_requires_session_id) {
    SessionMessage msg;
...
RB_TEST(session_read_rejects_bad_header) {
...
RB_TEST(session_read_rejects_unknown_kind) {
...
RB_TEST(session_read_rejects_missing_session_id) {
...
RB_TEST(session_read_tolerates_crlf_and_blank_lines) {
...
RB_TEST(session_inbox_path_helpers) {
    ...
    CHECK(is_session_inbox_path(".rocketbox-session/deadbeef.msg"));
    CHECK(!is_session_inbox_path("Incoming/file.bin"));
    CHECK(!is_session_inbox_path(".fabric-session/x.txt"));
}

RB_TEST(session_read_accepts_alt_header) {
    const std::string path = write_raw(
        "ROCKETBOX-SESSION-v1\nkind=announce\nsession_id=alt1\nfrom=Kyle\n");
    SessionMessage got;
    CHECK(read_session_file(path, got));
    CHECK(got.kind == SessionMessageKind::Announce);
    CHECK_STREQ(got.session_id, "alt1");
    std::remove(path.c_str());
}
