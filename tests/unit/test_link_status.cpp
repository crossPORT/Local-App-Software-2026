...
RB_TEST(link_status_busy_message) {
...
RB_TEST(link_status_connected_waiting_for_peers) {
    const StatusLine line = status_line_native(
...
RB_TEST(link_status_handshake_pending) {
    const StatusLine line = status_line_native(
...
RB_TEST(link_status_peer_expires_countdown) {
...
RB_TEST(link_status_peer_expiring) {
    using clock = std::chrono::steady_clock;
    const auto now = clock::now();
...
RB_TEST(link_status_transfer_done_message) {
...
RB_TEST(link_status_transfer_live_message) {
