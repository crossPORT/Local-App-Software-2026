...
std::string format_final_speed(double peak_mbps, double result_mbps, bool /*display_rate*/) {
...
std::string format_speed_line(double live_mbps,
    ...
    return format_final_speed(peak_mbps, result_mbps, display_rate);
}
...
wxString phase_title(Phase phase, bool usb_connected, bool has_peers, const std::string& status) {
    switch (phase) {
        case Phase::Idle:
            if (!usb_connected) {
...
void TransferProgressPanel::ApplyState(bool busy,
    ...
    const bool display_rate = display_rate_mib_s > 0.0;
    ...
    phase_label_->SetLabel(phase_title(phase, usb_connected, has_peers, status));
        ...
        case Phase::Transferring: {
            ...
            }
            {
                const std::string speed = format_speed_line(
                    live_mbps, peak_mbps, result_mbps, display_rate, true, outbound);
