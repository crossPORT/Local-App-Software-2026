...
describe('parseEventLogLine', () => {
  it('parses a structured line', () => {
    const parsed = parseEventLogLine(`${T} [port=2] announce_fail | transferOut timed out after 2500ms`);
  ...
  it('returns null for meta lines', () => {
    expect(parseEventLogLine(`${T} [boot] log cleared`)).toBeNull();
  });
});

describe('diagnoseEventLog', () => {
  it('returns nothing for a clean log', () => {
    ...
    expect(diagnoseEventLog(lines)).toEqual([]);
  });
  ...
  it('flags an announce wedge after consecutive transferOut timeouts', () => {
    ...
    const result = diagnoseEventLog(lines);
  ...
  it('does not flag a wedge below the threshold', () => {
    ...
    expect(diagnoseEventLog(lines).some((d) => d.id.startsWith('announce_wedge'))).toBe(false);
  });
  ...
  it('flags usb disconnected as the current skip reason', () => {
    ...
    expect(diagnoseEventLog(lines).some((d) => d.id === 'usb_disconnected:3')).toBe(true);
  });
  ...
  it('flags a flaky receive link after repeated frame failures', () => {
    ...
    const finding = diagnoseEventLog(lines).find((d) => d.id === 'flaky_receive:2');
    expect(finding).toBeDefined();
    expect(finding?.severity).toBe('warn');
  });
