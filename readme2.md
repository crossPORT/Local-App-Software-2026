...
|--------|------------------|---------|
| GTK app | `build/apps/gtk/data-transfer-demo` | Send / receive / loopback |
...
```
tools/*  ──┐
apps/gtk ──┼──► fabric_usb_core (core/) ──► libusb ──► FPGA 1772:0006
           │
           └── GTK only in apps/gtk (not in core or tools)
```

- **`core/`** — All USB protocol and transfer logic. No GTK, no platform UI.
- **`apps/gtk/`** — `MainWindow` + `TransferController`; implements [GUI_HANDOFF.md](GUI_HANDOFF.md) (worker thread, progress marshaling, error display).
