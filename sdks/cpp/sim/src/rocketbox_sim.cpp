...
class RocketBoxSimHub {
public:
    static RocketBoxSimHub& instance() {
        static RocketBoxSimHub hub;
        return hub;
    }
...
void rocketbox_sim_reset() {
    RocketBoxSimHub::instance().reset();
}
...
bool rocketbox_sim_device_bus_addr(int port_index, uint8_t* bus_out, uint8_t* addr_out) {
    if (!rocketbox_sim_port_available(port_index)) {
        return false;
    }
    if (bus_out) {
...
std::vector<RocketBoxUsbDevice> rocketbox_sim_list_devices() {
    return {
        RocketBoxUsbDevice{1, 10},
        RocketBoxUsbDevice{2, 11},
    };
}
...
TransferResult rocketbox_sim_send_file(const std::string& path,
    ...
    RocketBoxSimHub::instance().transmit(
    ...
    while (sent < file_size) {
        ...
        RocketBoxSimHub::instance().transmit(port_index, chunk.data(), static_cast<std::size_t>(n));
...
TransferResult rocketbox_sim_receive_file(const std::string& out_path,
    ...
    if (!RocketBoxSimHub::instance().rx(port_index).read(
...
TransferResult rocketbox_sim_loopback(const std::string& path,
    ...
    send_result = rocketbox_sim_send_file(path, send_port_index, progress_cb, usb_protocol::kFileTimeoutMs);
    receiver.join();
