...
int leg_from_serial(const std::string& serial) { return fabric_leg_from_serial(serial); }
...
std::vector<PresentPort> list_present_ports(libusb_context* ctx_in) {
    ...
    const auto devices = list_fabric_devices(g.ctx);
    ...
    for (size_t i = 0; i < devices.size(); ++i) {
        ...
        p.serial = fabric_device_serial(g.ctx, p.libusb_index);
        p.leg = leg_from_serial(p.serial);
        p.display_port = p.leg >= 0 ? display_port_from_leg(p.leg) : 0;
        p.available = fabric_port_available(g.ctx, p.libusb_index);
...
int auto_pick_libusb_index(libusb_context* ctx_in) {
    ...
    return resolve_fabric_port_index(g.ctx);
}
...
int libusb_index_for_display_port(int display_port, libusb_context* ctx_in) {
    ...
    for (const auto& p : ports) {
        if (p.display_port != display_port) {
            continue;
        }
        if (p.available) {
            return p.libusb_index;
        }
        if (fallback < 0) {
            fallback = p.libusb_index;
        }
    }
    return fallback;
}
