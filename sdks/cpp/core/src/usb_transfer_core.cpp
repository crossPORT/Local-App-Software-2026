...
std::vector<RocketBoxUsbDevice> list_rocketbox_devices(libusb_context* ctx) {
    std::vector<RocketBoxUsbDevice> devices;
    ...
    std::sort(devices.begin(), devices.end(),
              [](const RocketBoxUsbDevice& a, const RocketBoxUsbDevice& b) {
                  if (a.bus != b.bus) {
                      return a.bus < b.bus;
                  }
...
int count_rocketbox_devices(libusb_context* ctx) {
    return static_cast<int>(list_rocketbox_devices(ctx).size());
}
...
int resolve_rocketbox_port_index(libusb_context* ctx) {
    ...
    for (int index = 0; index < static_cast<int>(devices.size()); ++index) {
        if (rocketbox_port_available(ctx, index)) {
            return index;
        }
    }
    return 0;
