...
std::string default_event_log_path() {
    return (std::filesystem::path(temp_directory()) / "rocketbox-event.log").string();
}
...
std::string default_identity_config_path() {
    ...
    return (std::filesystem::path(home) / ".config" / "rocketbox" / "session.conf").string();
}

std::string shared_session_path_config() {
    return (std::filesystem::path(temp_directory()) / "rocketbox-session-path.conf")
        .string();
}
