...
// Wire write stays FABRIC-SESSION-v1 (last known-good interop with PWA).
// Dual-read ROCKETBOX-SESSION-v1 for any newer peers.
constexpr char kSessionHeader[] = "FABRIC-SESSION-v1\n";
constexpr char kAltSessionHeader[] = "ROCKETBOX-SESSION-v1\n";
...
bool parse_line(const std::string& line, SessionMessage& out) {
...
bool is_session_inbox_path(const std::string& relative_path) {
    const bool primary = relative_path.rfind(".fabric-session/", 0) == 0;
    const bool alt = relative_path.rfind(".rocketbox-session/", 0) == 0;
    if (!primary && !alt) {
        return false;
    }
    return relative_path.size() > 16
        && relative_path.compare(relative_path.size() - 4, 4, ".msg") == 0;
}

std::string write_session_temp_file(const SessionMessage& message) {
    ...
    return platform::write_temp_text_file("rocketbox-session-", text);
}

bool read_session_file(const std::string& path, SessionMessage& out) {
    ...
    if (payload.rfind(kSessionHeader, 0) == 0) {
        header_len = sizeof(kSessionHeader) - 1;
    } else if (payload.rfind(kAltSessionHeader, 0) == 0) {
        header_len = sizeof(kAltSessionHeader) - 1;
    } else {
        return false;
    }

    out = SessionMessage{};
    std::istringstream lines(payload.substr(header_len));
    std::string line;
