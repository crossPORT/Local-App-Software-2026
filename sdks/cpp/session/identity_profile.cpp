...
void apply_identity_key(IdentityProfile& cfg, const std::string& key, const std::string& value) {
    ...
    } else if (key == "display_rate_mib_s" || key == "booth_display_mib_s") {
        try {
            cfg.display_rate_mib_s = std::stod(value);
            if (cfg.display_rate_mib_s < 0.0) {
                cfg.display_rate_mib_s = 0.0;
            }
        } catch (...) {
            cfg.display_rate_mib_s = 0.0;
        }
    } else if (key == "display_rate_jitter_pct" || key == "booth_display_jitter_pct") {
        try {
            cfg.display_rate_jitter_pct = std::stod(value);
            if (cfg.display_rate_jitter_pct < 0.0) {
                cfg.display_rate_jitter_pct = 0.0;
            }
        } catch (...) {
            cfg.display_rate_jitter_pct = 0.0;
        }
...
bool load_profile_file(const std::string& path, int port_index, IdentityProfile& out) {
    ...
    out.display_rate_mib_s = port_cfg.display_rate_mib_s > 0.0
                                 ? port_cfg.display_rate_mib_s
                                 : global.display_rate_mib_s;
...
bool save_identity_profile(const IdentityProfile& profile) {
    ...
    if (profile.display_rate_mib_s > 0.0) {
        file << "display_rate_mib_s=" << profile.display_rate_mib_s << '\n';
    }
