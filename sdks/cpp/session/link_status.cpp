...
StatusLine status_line_native(bool usb_connected,
                              int devices_seen,
                              int port_index,
                              bool busy,
    ...
    if (usb_connected && identity_configured && !peers_configured) {
...
ConnectBannerLine connect_banner_native(bool usb_connected, int /*port_index*/) {
    if (usb_connected) {
        return {"", false};
    }
