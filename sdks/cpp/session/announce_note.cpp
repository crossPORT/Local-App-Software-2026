...
int leg_from_wire_port(int wire_port) {
    if (wire_port >= 1 && wire_port <= kPortCount) {
        return wire_port - 1;
    }
    if (wire_port >= 0 && wire_port < kPortCount) {
        return wire_port;
    }
    return -1;
}
...
int resolve_remote_port(int my_port, int announced_port) {
    if (announced_port == my_port) {
        return -1;
    }
    if (announced_port < 0 || announced_port >= kPortCount) {
        return -1;
    }
