...
import { countSimDevices } from './rocketbox_attach';
import type { ConnHooks } from './rocketbox_wire';
import type { SessionMessage } from './session_types';
import type { RocketBoxTransport, ListenMode, AnnouncePresenceMode } from './types';
...
export class RocketBoxTransportImpl implements RocketBoxTransport {
  ...
  describeDevice(): string {
    ...
    return formatPortLabel(this.getPortIndex(), this.getSerialNumber());
  }
  ...
  subscribeSession(handler: (m: SessionMessage) => void): () => void {
  ...
  switchDest = (): number => this.hw?.switchDest() ?? 0;
  async clearCircuit(): Promise<void> {
    if (!this.simulate && this.hw) await this.hw.switchPort(0);
  }
  sendSessionMessage = (m: SessionMessage) => this.io().sendSessionMessage(m);
  async sendAnnouncePresence(message: SessionMessage, mode: AnnouncePresenceMode): Promise<void> {
    if (this.simulate) {
      // WS fabric sees all ports; single send matches HW fire-and-forget presence.
      await this.io().sendSessionMessage(message);
      return;
    }
    await this.hw!.sendAnnouncePresence(message, mode);
  }
  sendBytes = (
    p: Uint8Array,
