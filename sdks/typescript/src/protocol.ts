...
export function displayMibSWithJitter(base: number, jitterPct: number, roll01: number): number {
  if (base <= 0) {
    return 0;
  }
  if (jitterPct <= 0) {
...
export function rollDisplayMibS(base: number, jitterPct: number): number {
  return displayMibSWithJitter(base, jitterPct, Math.random());
}
