...
import { attachSession, type AttachSessionOptions } from './rocketbox_attach';
...
export async function attachOrReplace(
  ...
  const opts: AttachSessionOptions = {
    simulate: bag.simulate,
    port: bag.port,
    device: bag.device,
  };
  const attached = await attachSession(opts);
